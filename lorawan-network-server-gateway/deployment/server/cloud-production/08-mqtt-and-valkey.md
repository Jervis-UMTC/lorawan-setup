...
### Current gateway-facing MQTT authentication correction - 2026-08-28

The target design earlier in this manual shows the gateway-facing Mosquitto backend `:8884` with `require_certificate true` and `use_identity_as_username true`. Later live inventory is authoritative for the current runtime: both brokers presently expose `:8884` with `require_certificate=false`, `allow_anonymous=false`, no `use_identity_as_username`, and no gateway-specific ACL/plugin. Treat the earlier mTLS block as the **target configuration**, not proof that gateway client-certificate authentication is already commissioned.

The cloud gateway mTLS boundary is therefore still open even though the broker TLS/failover foundation is healthy. Before public `mqtt.<DOMAIN>:8883` is activated, commission the gateway authentication boundary canary-first on `ulc-01` then `ulc-02`: issue one cloud MQTT `clientAuth` certificate whose CN is the authoritative Gateway EUI `0016c001f139a1cb`, add an exact-EUI ACL for the `as923/gateway/0016c001f139a1cb/...` event/state/command hierarchy, enable client-certificate authentication on the gateway-facing `:8884` listener, and prove positive mTLS plus cross-EUI/no-certificate denial through the existing anchor `:8883` TLS-passthrough path. This work does not require the final public FQDN or Reserved IPv4 and should be completed before provider public activation.
