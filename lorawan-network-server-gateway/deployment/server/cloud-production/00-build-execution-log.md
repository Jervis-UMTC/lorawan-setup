...
Do **not** yet record full `EVIDENCE_OBJECTSTORE=PASS`. S9 remains intentionally open because the accepted production `gateway-evidence-ingest objectstore-contract-write` plus cross-host `objectstore-contract-verify` must still execute. Discovery proved no accepted ingest binary/image is currently present on ulc-01/02/03. **Historical note:** at this checkpoint the working tree was believed to be untracked/not recoverable from `origin/main`; the 2026-09-01 whole-project audit later proved the full `evidence-services/` source is now tracked and present in `origin/main`. The accepted artifact is still not on the servers, so the exact binary-lock gate remains mandatory.
...
Second CONNECT-authority harness attempt - 2026-09-01: **HARNESS-ONLY STOP / NO DATABASE ACL MUTATION.** The corrected boolean-output version re-proved the Patroni topology (`ulc-01` leader, `ulc-02/03` replicas) and then failed on its first remote SQL call before any catalog mutation. The SSH transport embedded a multi-line remote command in which `docker exec -i spilo` and `psql ...` were separated by literal newlines without continuation, so the remote shell executed `docker exec -i spilo` without a command, then attempted host-side `psql`, producing `docker: 'docker exec' requires at least 2 arguments`, the pg_wrapper client warning, and `-U: command not found`. This is a transport-wrapper defect only; the `GRANT CONNECT` transaction was never reached, no password was regenerated, and PgBouncer/HBA/PostgreSQL service state were untouched. The next retry must carry SQL over SSH stdin directly to one single-line remote `sudo docker exec -i spilo psql ...` command; do not use `ssh -n` for that stdin-carrying path.

CONNECT-authority correction - 2026-09-01: **PASS.** The third guarded attempt fixed the SSH stdin transport and completed the intended single database-ACL mutation. Before mutation, the exact matrix was `t|t|t|t|f|f|f|f` for `PUBLIC_denied|telemetry_writer|telemetry_reader|fabric_adapter|gateway_evidence_ingestor|gateway_evidence_collector|gateway_evidence_verifier|evidence_ingest_ulc01`. Under `SET LOCAL ROLE telemetry_admin`, PostgreSQL granted `CONNECT ON DATABASE lorawan_telemetry` only to the three evidence NOLOGIN authority shells. The post-mutation matrix became `t|t|t|t|t|t|t|t` on `ulc-01`, `ulc-02`, and `ulc-03`, proving `PUBLIC` remains denied and the group-based admission replicated to every Patroni member. The existing protected `evidence_ingest_ulc01` credential was reused without regeneration; a fresh direct SCRAM + `sslmode=verify-full` session returned `evidence_ingest_ulc01|lorawan_telemetry|10.104.0.2|10.104.0.8|f|t|TLSv1.3|TLS_AES_256_GCM_SHA384`, and the workload role remained exactly `t|f|f|f|f|t|f|t|1|t`. Final Patroni topology remained one leader plus two replicas. Record `EVIDENCE_DATABASE_CONNECT_AUTHORITY=PASS`, `THREE_NODE_CONNECT_ACL_REPLICATION=PASS`, `EVIDENCE_INGEST_ULC01_DIRECT_AUTH=PASS`, `PASSWORD_REGENERATED=NO`, and `PGBOUNCER_CHANGED=NO`.

Evidence LOGIN identity issuance safe-resume - 2026-09-01: **COMPLETE / PASS.** The retry started from `existing_evidence_login_count=1`, preserved `evidence_ingest_ulc01` and its existing protected pending credential, and created only the five absent workload identities. Each of the six final roles is LOGIN + INHERIT, non-superuser, non-CREATEDB, non-CREATEROLE, non-REPLICATION, non-BYPASSRLS, has a SCRAM-SHA-256 verifier, exactly one intended NOLOGIN authority membership, and inherited CONNECT on `lorawan_telemetry`. Every role completed a fresh direct `sslmode=verify-full` session from `ulc-03` to the current leader `10.104.0.2`, returning TLSv1.3 / `TLS_AES_256_GCM_SHA384`. The protected credential set is six files under `/root/evidence-db-bootstrap`, all `0600 root:root`, 65 bytes, and all six secrets are unique. Three-node catalog verification returned `6|6|6|6|6|6` on `ulc-01`, `ulc-02`, and `ulc-03`. PgBouncer remained deliberately unchanged at four entries / zero evidence entries on all three nodes. Record `EVIDENCE_LOGIN_IDENTITY_ISSUANCE=PASS`, `EVIDENCE_LOGIN_ROLE_COUNT=6`, `EVIDENCE_LOGIN_SCRAM_COUNT=6`, `EVIDENCE_LOGIN_EXACT_MEMBERSHIP_COUNT=6`, `EVIDENCE_LOGIN_CONNECT_COUNT=6`, `EVIDENCE_LOGIN_DIRECT_VERIFY_FULL_AUTH=PASS`, `EVIDENCE_LOGIN_THREE_NODE_REPLICATION=PASS`, `UNIQUE_PENDING_SECRET_COUNT=6`, `EXISTING_EVIDENCE_INGEST_ULC01_ROTATED=NO`, and `PGBOUNCER_CHANGED=NO`.

PgBouncer evidence SCRAM rollout `ulc-01` canary - 2026-09-01: **PASS.** Baseline was `0640 root:postgres`, four entries, zero evidence entries, active/enabled, PID `1698991`. A ten-entry candidate was regenerated from authoritative local `pg_authid`; all ten entries were SCRAM and the existing `chirpstack`, `fabric_adapter`, `telemetry_reader`, and `telemetry_writer` verifier lines remained byte-identical. The prior userlist was preserved under `/etc/pgbouncer/evidence-userlist-rollout-20260901T004328Z`; old/rollback SHA-256 was `e159c1c3e408eb091629083a81738c730bb8d2ab3c07ecbc6aaf109c51b9d5e8`, candidate SHA-256 `665f1592c96ca276681a454b9cbcd6ab8ab0cbfb4594b8ddd443239db58df391`. The candidate installed as `0640 root:postgres`, PgBouncer reloaded in place with PID `1698991` unchanged, and final state was ten entries/six evidence entries. From `ulc-03`, strict verify-full authentication through physical endpoint `10.104.0.2:6432` succeeded for `evidence_ingest_ulc01` and `evidence_collector_ulc01`, both reaching writable `lorawan_telemetry`. `ulc-02` remained `4|0|active|1512906` and `ulc-03` remained `4|0|active|1690020`. Patroni stayed one leader/two replicas. Record `ULC01_PGBOUNCER_EVIDENCE_SCRAM_REFRESH=PASS`, `EXISTING_FOUR_VERIFIER_PARITY=PASS`, `ULC01_PGBOUNCER_PID_UNCHANGED=PASS`, `ULC02_PGBOUNCER_CHANGED=NO`, `ULC03_PGBOUNCER_CHANGED=NO`, and `PASSWORD_ROTATION=NO`.

Whole-project evidence/gateway handoff audit - 2026-09-01: the full `evidence-services/` source tree is now tracked and `origin/main` contains the gateway/cloud source; earlier statements that it was untracked are obsolete. The accepted production cloud artifacts are still not staged on the servers, so binary-lock verification remains mandatory before SeaweedFS S9/OCI deployment. Gateway Rust inspection confirms the current crate is a core/library rather than a deployable journal service: no executable `main`, live Concentratord ZMQ/IPC client, filesystem/fsync persistence implementation, or HTTP/mTLS transport exists yet. New-chat efficiency order is therefore: start the pinned gateway Rust build/cache immediately, then continue `ulc-02 -> ulc-03` PgBouncer evidence rollout while implementing the missing gateway runtime/package layers.

Exact resume: read `00-evidence-services-and-gateway-journal-continuation.md`. Start the gateway Rust build/cache first. Cloud lane resumes at **ulc-02 PgBouncer evidence SCRAM refresh**, then ulc-03; do not repeat the accepted ulc-01 canary.

### Automation-runner SSH authority recheck - 2026-09-01

The next automated cloud attempt rechecked transport before touching `ulc-02`. TCP/22 remained reachable on all three public Droplet addresses, and SSH advertised **publickey only**, matching the hardened policy. The workstation still contains encrypted `id_ed25519` and `id_ed25519_home_ops`; repository history proves `id_ed25519_home_ops` previously authenticated the cloud path as `jervis`, but its key passphrase is not available to this runner. No SSH agent is available and the dedicated `id_ed25519_lorawan_ops` identity is absent. No password-SSH/root-login/host-key policy was changed and no server command or PgBouncer mutation executed. Record `AUTOMATION_SSH_TRANSPORT_REACHABLE=PASS`, `AUTOMATION_SSH_AUTHORITY=BLOCKED_BY_LOCKED_APPROVED_KEY`, and `SERVER_MUTATION=NO`. Cloud resume remains `ulc-02 -> ulc-03` once that already-authorized key is securely unlocked/loaded.

### Gateway Rust pinned fallback revalidation - 2026-09-01

Because enterprise `MachinePolicy=Restricted` blocks the canonical PowerShell wrapper, the ignored project-local Rust fallback was restored without changing machine policy. `RUSTUP_HOME` lives under `.dev-cargo/rustup`; exact toolchain identity is Rust `1.82.0`, rustc commit `f6e511eec7342f59a25f7c0534f1dbea00d01b14`, host `x86_64-pc-windows-gnu`. Fresh default-feature gates passed: `cargo fmt --all -- --check`; `cargo test --locked` with **13 passed / 0 failed**; `cargo clippy --locked --all-targets -- -D warnings`; and `cargo build --locked`. Representative `gateway-evidence-writer --check-config` and `gateway-evidence-uploader --check-config` probes both returned PASS using the frozen Gateway EUI/event endpoint, finite journal policy, HTTPS ingest, mTLS path configuration, and bounded retry settings. No credential contents were printed.

This supersedes the earlier audit statement that the gateway crate had no executable entry points or Concentratord transport source. Writer/uploader configuration executables now exist and the optional `concentratord-zmq` source exists. The optional native ZMQ feature is **not** claimed compiled for the final target from this workstation; that feature still belongs to the Gateway OS/OpenWrt-capable build boundary. Next gateway implementation is crash-safe atomic/fsync state + segment persistence, then durable receipt files, HTTP/mTLS execution, and package/service supervision. Record `GATEWAY_RUST_PINNED_TOOLCHAIN=PASS`, `GATEWAY_RUST_DEFAULT_TESTS=13_PASS`, `GATEWAY_RUST_CLIPPY=PASS`, `GATEWAY_RUST_BUILD=PASS`, and `GATEWAY_WRITER_UPLOADER_CONFIG=PASS`.

### Automation SSH recovery + three-node PgBouncer evidence closure - 2026-09-01

The prior automation-credential stop is superseded. Exact key/user verification proved the approved encrypted workstation identity `id_ed25519_home_ops` is authorized as `opsadmin` on all three active Droplets; earlier attempts as `jervis` used the wrong account. The approved key was loaded into a user-space Linux SSH agent on `cloud-sim`, with the existing ULC host keys pinned and strict host-key checking preserved. Password SSH, root SSH, and enterprise workstation policy were not weakened. `cloud-sim` also provides a working Linux Docker/Buildx build path while the company workstation remains `MachinePolicy=Restricted`.

Live PgBouncer inspection showed the server state had already advanced beyond the earlier checkpoint: `ulc-01`, `ulc-02`, and `ulc-03` all carry the exact same `0640 root:postgres`, 1582-byte, ten-entry/six-evidence userlist SHA-256 `665f1592c96ca276681a454b9cbcd6ab8ab0cbfb4594b8ddd443239db58df391`. The old four-role userlist SHA is `e159c1c3e408eb091629083a81738c730bb8d2ab3c07ecbc6aaf109c51b9d5e8`. Rollback directories are `/etc/pgbouncer/evidence-userlist-rollout-20260901T004328Z`, `...T020032Z`, and `...T020244Z` respectively. `ulc-02` and `ulc-03` rollback evidence proves all four original verifier lines remained byte-identical. Current PgBouncer PIDs `1698991`, `1512906`, and `1690020` equal the recorded pre-expansion PIDs, proving in-place reload without process restart.

Final application acceptance used protected root-only evidence credentials without printing them. Strict `sslmode=verify-full` sessions through physical PgBouncer endpoints `10.104.0.2:6432`, `10.104.0.4:6432`, and `10.104.0.8:6432` authenticated as commissioned evidence identities, negotiated TLSv1.3 / `TLS_AES_256_GCM_SHA384`, and all reached writable `lorawan_telemetry` on current Patroni primary `10.104.0.2`. Record `AUTOMATION_SSH_AUTHORITY=PASS`, `THREE_NODE_PGBOUNCER_EVIDENCE_SCRAM=PASS`, `THREE_NODE_USERLIST_PARITY=PASS`, `PGBOUNCER_RESTART=NO`, and `PASSWORD_ROTATION=NO`. Exact cloud resume is SeaweedFS S9 using the locked `gateway-evidence-ingest` artifact; do not repeat PgBouncer expansion without a later credential change.

### Gateway-evidence cloud commissioning COMPLETE / SERVER PASS - 2026-09-01

The remaining cloud evidence boundaries were completed without weakening the commissioned SSH, TLS, database, broker, or container-security model. The accepted Linux/amd64 binaries were reproduced against the frozen lock, packaged as minimal scratch images, published under the authenticated personal GHCR namespace, and installed using immutable references: ingest `ghcr.io/jervis-umtc/lorawan/gateway-evidence-ingest@sha256:f2fd695e8d2505a12202b4fabd92d035c562e16d4e308ccf9436a08a36aaec1a`; collector `.../gateway-mqtt-evidence-collector@sha256:0bcaaa127f913157596a87077235d4f0717fa1c29f08d8aca8568c3167ed2cff`; verifier `.../gateway-evidence-verifier@sha256:3416496fa563154fbd7209a575acd040b735b3c4f4159902fc886e78167195c0`; adapter `.../gateway-fabric-adapter@sha256:cd4308e8985d74ea7fab957a2a4adadd1831b776a8f9af2fdd6291179df83e7a`. All three repository preflights passed against the real protected host env/PKI inputs.

SeaweedFS S9 executed through the production `gateway-evidence-ingest` helper at `https://evidence-objects.internal.lorawan.com:18443`. The create-only acceptance/race fixtures were retained and cross-host verification from `ulc-03` passed, closing `OBJECTSTORE_CONTRACT=PASS`, `SEAWEEDFS_S9_WRITE=PASS`, `SEAWEEDFS_S9_CROSS_HOST_VERIFY=PASS`, and `EVIDENCE_OBJECTSTORE=PASS`.

Evidence PKI and MQTT authorization were commissioned separately. Four distinct collector mTLS identities were installed for collector-1/2 against broker-1/2 with read-only `as923/gateway/+/event/#` ACLs. Live placement became `ulc-01=ingest-1+collector-1+adapter-1-disabled`, `ulc-02=ingest-2+verifier-1+adapter-2-disabled`, and `ulc-03=collector-2+verifier-2`. Ingest readiness proves PostgreSQL plus object-store reachability; collector readiness requires both physical Mosquitto mTLS sessions plus PostgreSQL/object storage; verifier readiness includes PostgreSQL/object storage and the pinned trusted-decoder self-test. Both Fabric adapters run only with `FABRIC_ADAPTER_ENABLED=false`; no OpenBao SecretID or Fabric client activation occurred.

Runtime security inspection on every evidence container returned numeric `65532:65532`, read-only rootfs, `cap_drop=ALL`, `no-new-privileges`, PID limit `128`, memory `201326592` bytes, NanoCPUs `200000000`, bounded logs and `restart=unless-stopped`. Docker is active/enabled on all three hosts.

A controlled AS923 gateway-style `event/up` using Gateway EUI `0016c001f139a1cb` then exercised the real broker/collector persistence boundary. The positive gateway identity publish produced exactly one deduplicated MQTT witness carrying the expected PHY hash, uplink ID `16909060`, frequency `923200000`, RSSI `-72`, SNR `8.5`, correlation digest and immutable object reference. A collector-identity publish probe did not create a second capture, proving the read-only ACL boundary in application state even though the command-line publisher's local exit status alone was not an authorization oracle.

The existing Phase 10 anchor `:443` design was upgraded one host at a time into a shared TCP/SNI dispatcher with timestamped rollback copies. The `10.15.0.5` and `10.15.0.7` addresses were re-confirmed as DigitalOcean host-local anchor addresses, not the `10.104.0.x` east-west VPC. `evidence.internal.lorawan.com` now passes TLS unchanged to each host's private ingest `:18100`; `chirpstack.internal.lorawan.com` routes to a loopback `127.0.0.1:14443` TLS terminator preserving the previous ChirpStack HTTP backend. Both HAProxy configurations validated/reloaded cleanly, ChirpStack remained HTTP `200`, evidence without a client certificate was rejected with `certificate required`, and one representative gateway mTLS request traversed the shared `:443` layer to ingest readiness. Record `SHARED_443_SNI=PASS` and `EVIDENCE_SHARED_443_MTLS=PASS`. Public Internet activation is **not** claimed: a real domain, same-region Reserved IPv4, provider firewall/DNS and public certificates remain external inputs.

Grafana on `ulc-03` retained its loopback-only `13.2.0` runtime and strict-TLS `telemetry_reader` datasource. Database ACL checks proved `telemetry_reader` has schema usage plus SELECT on `gateway_evidence.checkpoint_status` and `gateway_evidence.verification_status`. The file-provisioned `LoRaWAN Telemetry Overview` was extended from four to six panels by adding `Evidence checkpoints` and `Evidence verification state`; Grafana's provisioning watcher loaded all six without a container restart. The first API check used the wrong admin environment variable names and returned `401`; after resolving only the variable **names**, the corrected protected API call returned `GRAFANA_EVIDENCE_PANEL_COUNT=6` with the expected two new titles. No Grafana credential was exposed.

For future physical-gateway staging, `ulc-03` now holds one consolidated protected MQTT + Evidence identity tree for Gateway EUI `0016c001f139a1cb` under `/root/lorawan-gateway-handoff/0016c001f139a1cb/`. An AES-256-CBC/PBKDF2 encrypted transfer artifact exists at `/root/lorawan-gateway-handoff/gateway-handoff-0016c001f139a1cb.tar.gz.enc`, SHA-256 `6e957385f9e4467b1924368a16a719c27d50cd0f90941435e420636af2ec94d3`; source private keys remain `0600 root:root`, and authoritative issuer copies remain retained for revocation/recovery. This is a server-side handoff artifact only, not a claim that identities are installed on the physical gateway.

Finally, the current Rust gateway source was revalidated because the implementation had advanced beyond older Markdown. With pinned Rust `1.82.0`, exact rustc commit `f6e511eec7342f59a25f7c0534f1dbea00d01b14`, the fresh default gate passed `cargo fmt`, `cargo test --locked` with **28 total tests / 0 failed** (19 unit + 9 contract), Clippy with warnings denied, and locked build. The source now includes crash-safe `PersistentJournal`, durable canonical `ReceiptStore`, HTTPS/mTLS curl transport, receipt validation/idempotency, `--sync-once`, continuous uploader loops and bounded retry/backoff. Local evidence deletion/retirement remains intentionally absent. The next implementation boundary is target-native `concentratord-zmq` compilation and the existing OpenWrt UCI/procd package in the pinned Gateway OS toolchain, followed by one real physical journal/MQTT/application lineage.

Record `EVIDENCE_CLOUD_RUNTIME=PASS`, `GATEWAY_EVIDENCE_RUNTIME=SERVER_PASS_GATEWAY_PENDING`, and `GATEWAY_EVIDENCE_V2_NORMAL_PATH=NOT_YET_CLAIMED`. Fabric remains disabled until the real external handoff and deliberate credential activation.

### Public ingress normal path + certificate HA automation - 2026-09-01

This later boundary supersedes the immediately preceding statement that public Internet activation was not yet claimed. DigitalOcean Reserved IPv4 `129.212.208.168` is live and currently owned by `ulc-01`. Public service identities are `smartagri-chirpstack.duckdns.org:443`, `smartagri-evidence.duckdns.org:443`, and `smartagri-mqtt.duckdns.org:8883`.

ChirpStack public HTTPS passed normal system-trust validation with a Let's Encrypt certificate. The Evidence public endpoint presented the intended public server identity, rejected a client with no certificate, and accepted the real Gateway EUI Evidence client through mTLS to `/readyz`. The MQTT public endpoint passed strict hostname verification and the real gateway MQTT identity completed client-auth TLS, MQTT `CONNACK 0`, and an authorized subscription on its command hierarchy. No anonymous/public-client downgrade was introduced: MQTT and Evidence retain their dedicated private client-auth CAs while their server certificates include the public DuckDNS SANs.

ChirpStack certificate renewal is now operationally HA-aware. `ulc-01` remains the ACME owner while the Reserved IPv4 is there; a Certbot deploy hook synchronizes the renewed PEM to `ulc-02` through a dedicated source-restricted SSH key authenticated as `opsadmin`, a forced receiver command, and an exact sudoers allowance. `PermitRootLogin no` remains intact. Manual synchronization and HTTPS through the standby anchor passed, and a periodic reconciliation timer provides recovery if the deploy-hook transfer is missed. A daily read-only certificate-expiry checker/timer is installed and tested on all three hosts and follows Certbot live-lineage symlinks.

The public normal path did not change Node-RED or Fabric execution semantics: Node-RED A remains active, B remains fenced, the previously accepted atomic-outbox/replay proof stays authoritative, both Fabric adapters remain `FABRIC_ADAPTER_ENABLED=false`, and no adapter SecretID was issued.

The one public-provider boundary intentionally left open is Reserved-IP mobility. A suitable least-privilege DigitalOcean API identity and a controlled ulc-01 <-> ulc-02 reassignment/failover acceptance are still required before automatic public-ingress failover is claimed. Record `PUBLIC_CHIRPSTACK_443=PASS`, `EVIDENCE_PUBLIC_443=PASS`, `PUBLIC_MQTT_8883=PASS`, `PUBLIC_INGRESS_NORMAL_PATH=PASS`, and `PUBLIC_RESERVED_IP_FAILOVER=EXTERNAL_AUTH_PENDING`.
