...
> **Status: SERVER-ONLY STAGING COMPLETE / PASS; REAL-DATA ACCEPTANCE HARDWARE DEFERRED.** Grafana `13.2.0` is running on `ulc-03` from immutable digest `sha256:3fd54ae1214669f8355f065ec9f6445d5279a3d77095ab048ca045685272429b`, loopback-only on `127.0.0.1:3000`, with strict-TLS `telemetry_reader` datasource health and read-only ACL proof PASS plus the four-panel dashboard provisioned. `GRAFANA_SERVER_STAGING=PASS` is authoritative. Full Phase 14A still requires a real EMU-01 payload-v2 row and freshness/latest-reading dashboard correlation. Grafana is not part of the LoRaWAN control path.
