...
```yaml
  grafana:
    ...
    cpus: "${LAB_GRAFANA_CPUS}"
    mem_limit: "${LAB_GRAFANA_MEM}"
    ports:
      - "127.0.0.1:3000:3000"
    environment:
...
Confirm the top-level `grafana-data` volume already declared in [Server 2](../ha-cluster/02-docker-topology-and-network.md). Do not add a second top-level `volumes:` key.
