...
You can complete this verification with zero sensor rows. The goal is to prove that the exact database, roles, hypertables, uniqueness rules, views, retention policy state, and generic measurements path are ready for the first real device. A running or healthy container alone is insufficient evidence.

### Cloud HA POC command adaptation
...
Interpret the result against the selected retention policy:

- if retention was explicitly approved and enabled, there should be one matching job per protected time-series table;
- if the cloud POC intentionally leaves retention disabled, **zero retention jobs is valid** and must be recorded as intentional;
- never assume old rows will be deleted merely because TimescaleDB is enabled.

Do not add a destructive retention job just to make this verification section return rows.
