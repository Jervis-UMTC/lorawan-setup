...
> **Current cloud HA state:** active/passive Node-RED server commissioning is complete. Node-RED A on `ulc-03` is healthy/active; Node-RED B on `ulc-02` is fully staged and stopped/fenced. Both candidates map `mqtt.internal.lorawan.com` and `pgbouncer.internal.lorawan.com` to their own host (`10.104.0.8` for A, `10.104.0.4` for B), both private `:18884` MQTT frontends and dedicated `:8886` mTLS identities/ACLs are commissioned, and both carry the same reviewed runtime/flow revision. Keep B stopped until an approved fenced promotion. Real EMU-01 ingestion/replay acceptance remains hardware-dependent. Do not use the obsolete `mqtt-ha.internal.<DOMAIN>:18883` design or the ChirpStack-specific `:18883 -> :8885` route.
