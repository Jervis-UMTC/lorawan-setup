...
Gateway OS Base contains the lightweight runtime needed for Concentratord and MQTT Forwarder with an external ChirpStack server. Keep the legacy UDP Forwarder disabled. Do not use Gateway OS Full for this test track because it adds a local ChirpStack server that the test VM already provides.
...
* **Base vs. Full Variant:** Gateway OS Base provides the radio and forwarding services required when ChirpStack runs on another machine. The test track adds only the local Mosquitto buffer on the gateway; ChirpStack, Node-RED, databases, OpenBao, and Fabric integration stay off the Raspberry Pi.
