...
For this test setup, MQTT Forwarder must not connect directly to the remote broker. It sends QoS 1 messages to the loopback Mosquitto broker, which owns persistent buffering and the remote mTLS bridge.

Complete [04-configure-local-mqtt-buffer.md](04-configure-local-mqtt-buffer.md) before continuing.
...
Region topic prefix: as923 (use `<REGION_TOPIC_PREFIX>` only when adapting this manual to another approved deployment)
...
Do not enter `<MQTT_BROKER_FQDN>` on this page. Doing so bypasses the persistent local queue.
...
The subscription should display a topic for the configured Gateway EUI. Healthy local traffic commonly includes `event/stats`; the Forwarder also subscribes to the gateway `command/+` hierarchy and local retained connection state can appear at `state/conn`. Payloads are Protobuf and may look like binary/unreadable text in `mosquitto_sub`; that is normal. The important check is the correct `as923/gateway/<GATEWAY_EUI>/...` topic and message arrival.
