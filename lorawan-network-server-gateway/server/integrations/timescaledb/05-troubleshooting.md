...
Diagnose from the database outward and change one layer at a time. Run the shell commands in this guide on the LoRaWAN application server from `/opt/chirpstack-docker`. Before editing, capture the Compose project, image reference, volume name, PostgreSQL and TimescaleDB versions, schema version, free disk, memory, service state, and last 200 relevant log lines. Do not immediately change ChirpStack, Node-RED, and Grafana together.
