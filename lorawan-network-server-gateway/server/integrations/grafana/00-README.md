...
Run Docker and database commands on the LoRaWAN application server from `/opt/chirpstack-docker`. Perform Grafana menu and panel actions in a browser connected through the documented SSH tunnel. The browser reaches Grafana; it does not connect directly to PostgreSQL.
