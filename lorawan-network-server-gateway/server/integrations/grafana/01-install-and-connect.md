...
Replace `<PINNED_GRAFANA_IMAGE>` with a tested version tag or immutable digest before running Compose. If Grafana already exists, keep one service and add the telemetry-db dependency and persistent volume rather than creating a duplicate. Do not use a floating `latest` tag for a recoverable deployment.
