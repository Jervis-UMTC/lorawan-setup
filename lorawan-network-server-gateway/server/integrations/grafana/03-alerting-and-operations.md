...
Before the Dragino arrives, an empty dashboard is expected. Do not interpret no telemetry rows as a Grafana failure, and do not configure an empty result as `OK` without an explicit reason. Validate the data source with a connection test and validate the pipeline after the first real uplink.
