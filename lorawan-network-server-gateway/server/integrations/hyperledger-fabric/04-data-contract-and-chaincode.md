...
~~~json
{
  ...
  "evidence": {
    ...
    "reference": "<OFF_CHAIN_EVIDENCE_REFERENCE>",
...
Use one standards-based canonical JSON specification and compatible library in every participating implementation. Include the exact standard/version and cross-language test vectors in the contract so each team can prove identical canonical bytes and digests. Do not hash a language-native object through an unspecified serializer and assume another language will produce the same bytes.
