...
Node-RED subscribes to ChirpStack application events, validates and normalizes them, and writes telemetry plus any Fabric outbox job in one database transaction.
...
Do not add Fabric submission to the Node-RED flow. The flow only creates a durable outbox job for selected events; the adapter submits it later.
